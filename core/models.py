...
class LLMConfig(Base):
    """Central, persisted LLM configuration used by the runtime factory.

    The API key is stored encrypted at rest using a server-side key. The admin
    API never returns the secret value and the UI only reports whether it is configured.
    """
