...
class TestProviderSelection:
    ...
    def test_openai_custom_base_url_uses_generic_runtime_path(self, monkeypatch):
        ...
        assert not any(c.get("base_url") == PERPLEXITY_BASE_URL for c in calls)

    def test_custom_base_url_runtime_path_is_provider_name_agnostic(self, monkeypatch):
        for provider_name, base_url in (
            ...
            assert calls == [{"api_key": "test-openai-key", "base_url": base_url}]
    ...
    def test_persisted_admin_llm_config_overrides_env(self, monkeypatch):
        ...
        p = build_provider()
        assert p is not None and p.name == "openai"
        assert calls == [{"api_key": "db-openai-key", "base_url": "https://db.example.com/v1"}]

    def test_persisted_admin_local_config_used_when_selected(self, monkeypatch):
        monkeypatch.setenv("OPENAI_BASE_URL", "http://env-host/v1")
        monkeypatch.setenv("HLEO_LLM_PROVIDER", "openai")
        Base.metadata.create_all(bind=engine)
        with SessionLocal() as db:
            db.query(LLMConfig).delete()
            db.add(LLMConfig(provider="local", base_url="http://admin-host/v1", model="qwen2.5-7b-instruct"))
            db.commit()
        calls = _patch_openai_ctor(monkeypatch)
        p = build_provider()
        assert p is not None and p.name == "local"
        assert calls == [{"api_key": "local", "base_url": "http://admin-host/v1"}]

    def test_freeform_provider_name_uses_generic_openai_compatible_config(self, monkeypatch):
        Base.metadata.create_all(bind=engine)
        with SessionLocal() as db:
            db.query(LLMConfig).delete()
            db.add(LLMConfig(
        ...
        assert calls == [{"api_key": "openrouter-key", "base_url": "https://openrouter.ai/api/v1"}]
        assert not any(c.get("base_url") == "https://api.openai.com/v1" for c in calls)
        assert resolve_model("OpenRouter", "gpt-4o-mini") == "openrouter/free"

    def test_freeform_provider_requires_base_url(self):
        Base.metadata.create_all(bind=engine)
        with SessionLocal() as db:
            db.query(LLMConfig).delete()
            db.add(LLMConfig(
